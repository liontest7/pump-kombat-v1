...
create or replace function public.increment_user_wins(
  user_id uuid
)
returns void
language plpgsql
...
create or replace function public.increment_user_losses(
  user_id uuid
)
returns void
language plpgsql
...
comment on function public.increment_user_wins is 'Increments win count and ranking points for a user';
comment on function public.increment_user_losses is 'Increments loss count and decreases ranking points for a user';
